...
第一次接触本项目，请先看 [新手上手指引](docs/quickstart.md)。当前正在准备朋友可试用版；人工验收后移，内部验证继续进行。
...
当前目标调整为**朋友可试用版**。不要求用户现在临时提供需求或验收“已经变轻”，也不等到所有功能完善才开始验证。
...
| 4 朋友试用与反馈优化 | 验证陌生用户体验和业务结果 | 分享有限范围版本，观察卡点、求助和返工，按反馈优化 | 作者代操作掩盖问题、少数样本被过度推广 |

阶段 1 的工具验证通过，机制减负未验收；阶段 2 已有文字指引和依赖判断修正，其余入口能力待实现。历史样例尚未选定或执行。内部试验不能代替朋友试用，也不能自动产生效率提升结论。
